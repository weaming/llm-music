...
- **llm-server**：OpenAI Chat Completions 的透传代理。客户端要流式就逐帧原样转发，要非流式则在**内部**转发成流式、读完再按上游非流式响应的形状合成回去 —— 两条路径调用者的观感都与直连上游无异，而上游错误（429/401…）的状态码与信封一律原样回传。自身只做三件事：补默认 model、记用量、发旁路。旁路逐帧发布上游 SSE 原文，**只推实时事件、不做历史补发**，无订阅者时零开销，且绝不阻塞主链路。
...
之后任何打到 `:3050` 的请求都会变成声音，**包括非流式请求**（服务内部把它转发成流式，再把结果合成非流式响应回给客户端）。
